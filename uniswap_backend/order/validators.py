...
class SerializerValidatorMixin:
    ...
    def validate_token_count(to_count: Union[float, int], from_count: Union[int, float]):
        if to_count == 0:
            raise serializers.ValidationError({'to_count': 'must be greater than zero'})
        if from_count == 0:
            raise serializers.ValidationError({'from_count': 'must be greater than zero'})
    ...
    def validate_time_range(start_time: datetime, end_time: datetime):
        ...
        raise serializers.ValidationError({'start_time': 'Invalid time range', 'end_time': 'Invalid time range'})

    @staticmethod
    def validate_balance(token_from, quantity):
        token_input_address, token_decimals = token_check_address(token_from)
